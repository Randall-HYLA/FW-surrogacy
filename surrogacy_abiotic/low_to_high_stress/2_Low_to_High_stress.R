#Surrogacy analysis of FW_pecies as targets and water stress (low to high stress) as surrogates

library(sf); library(dplyr); library(foreach); library(forcats); 
library(raster);library(data.table);library(ggplot2)
...
fw_stress_matrix <- fw_stress_matrix %>% arrange((water_stress_raster_res50)) #organize from low to high values
