...
extract_zonation_maps <- function(taxon = "reptiles", algorithm = "CAZ", resolution = "100", out_dir = "output/zonation_runs", suffix = "0000"){
...
extract_zonation_ranks <- function(taxon = "reptiles", algorithm = "CAZ", out_dir = "output/zonation_runs"){
  ...
  ### Extract Zonation ranks
  rank_rasters <- purrr::map(dirfiles, raster::raster)
...
target_accumulation_CAZ <- function(target_matrix){
  cumulative_target <- target_matrix %>% mutate_all(cumsum)
  out_curves <- (rowSums(cumulative_target > 0))
...
#### Match algorithm
  algorithm <- match.arg(algorithm)
  if (is.null(n_cells)) 
    n_cells <- nrow(target_ranks$zonation_ranks)  
  #### Calculate range sizes
  if (algorithm == "ABF") 
    range_sizes <- colSums(target_matrix[, -c(1:2)])
  #### Update objects
  reference_coordinates <- target_ranks$reference_coordinates
  reference_coordinates$x <- as.character(round(reference_coordinates$x, digits = 2))
  reference_coordinates$y <- as.character(round(reference_coordinates$y, digits = 0))
  target_matrix$x <- as.character(round(target_matrix$x, digits = 2))
  target_matrix$y <- as.character(round(target_matrix$y, digits = 0))
  target_ranks <- target_ranks$zonation_ranks[1:n_cells, ]

  target_matrix <- dplyr::left_join(reference_coordinates, target_matrix, by = c("x", "y"))
...
get_sai_curves_surrogate <- function(target_matrix,
  ...
  target_matrix$x <- as.character(round(target_matrix$x, digits = 2))
  target_matrix$y <- as.character(round(target_matrix$y, digits = 0))
  surrogate_ranks <- surrogate_ranks$zonation_ranks[1:n_cells, ] #Check this part
  target_matrix <- dplyr::left_join(reference_coordinates, target_matrix, by = c("x", "y"))
  target_matrix[is.na(target_matrix)] <- 0
  
  head(target_matrix[,1])
...
calculate_sai <- function(sai_curves){
  ...
  for (i in 1:ncol(sai_curves$optimal)){
    sai_areas_optimal[i] <- caTools::trapz(area, c(0, sai_curves$optimal[, i]))
  }
  ### Area under Surrogate curves
  for (i in 1:ncol(sai_curves$surrogate)){
    sai_areas_surrogate[i] <- caTools::trapz(area, c(0, sai_curves$surrogate[, i]))
  }
  ### Area under Random curves
  for (i in 1:ncol(sai_curves$random)){
    sai_areas_random[i] <- caTools::trapz(area, c(0, sai_curves$random[, i]))
  }
...
biome_protected_target <- function(target_data, biome_cover, reference_raster = terrestrial_extent_raster){
  reference_df <- reference_raster %>% 
    as.data.frame(xy = TRUE) %>% 
    dplyr::filter(layer == 1) %>% 
    dplyr::select(x, y)
  biome_cover_df <- projectRaster(biome_cover, reference_raster) %>% as.data.frame(xy = TRUE) %>% 
    dplyr::rename(biome = layer) %>% 
    dplyr::filter(biome == 1) %>%
    mutate(cellID = cellFromXY(reference_raster, data.frame(x, y)))
  #### Calculate range sizes
